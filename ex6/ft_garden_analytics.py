class Plant():
    ...
    @staticmethod
    def comparaison_age(age: int) -> None:
        if age > 365:
...
class Tree(Plant):
    ...
    def __init__(self, name: str, height: float, ages: int,
        ...
        self._stats: "Tree.Stats" = self._stats
...
class Seed(Flower):
    def __init__(self, name: str, height: float, ages: int,
                 color: str, seed_c: int):
        super().__init__(name, height, ages, color)
        self.seed_c = seed_c

    def seed_compte(self) -> None:
...
def affiche_stat(plant: Plant) -> None:
